...
export class NewsDataProvider {
  ...
  url: string = 'http://nowizzville.com/api/events';
  ...
  if (this.data) {
     ...
     return this.cache.loadFromDelayedObservable(this.url, req, this.newsKey, this.ttl, delayType);
...
getArticles(articleIndex: number, queryText = '') {
    ...
    return this.loadEvents().map((data: any) => {
